...
const CurrencyView = () => {
  ...
  if (isLoading) {
    return (
      <div className="currency-view-container">
        <p className="currency-loading">Döviz kurları yükleniyor...</p>
      </div>
    );
  }
  ...
  if (isError && displayRates.length === 0) {
    return (
      <div className="currency-view-container">
        <p className="currency-error">Döviz kurları çekilemedi.</p>
      </div>
    );
  }
        ...
        <table className="currency-table">
          ...
          <tbody>
            {displayRates.map((rate) => (
              <tr key={rate.ccy}><td data-label="Currency">{rate.ccy}</td>

...
const generateDailyMockData = (liveBaseRate, days = 30) => {
    ...
    for (let i = 0; i < days; i++) {
        ...
        const rate = liveBaseRate + Math.sin(i * 0.4) * (fluctuation * 0.7) + (i % 5) * 0.0001; 
...
const CurrencyAreaChart = ({ liveBaseRate }) => {
    ...
    useEffect(() => {
        try {
            ...
            const dashboardTextColor = rootStyles.getPropertyValue('--color-dashboard-text').trim();
        ...
        } catch (e) { /* Hata yoksayıldı */ }
    }, []); 
    
    const historicalRates = useMemo(() => {
        const rates = generateDailyMockData(liveBaseRate || 30.00, 30);
        return rates;
    }, [liveBaseRate]);
    ...
    if (!historicalRates || historicalRates.length === 0) {
        return (
            <div className="currency-chart-inner-wrapper">
                 <p style={{textAlign: 'center', color: 'gray', padding: '10px'}}>Veri bekleniyor...</p>
            </div>
        );
    }

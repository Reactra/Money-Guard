...
const AddTransactionModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isExpense && (!incomeCategory || !incomeCategory.id)) {
      toast.error("Gelir kategorisi API'den yüklenemedi. İşlem eklenemiyor.");
      return;
    }
    ...
    console.log("API'ye GİDEN Payload (Son):", newTransaction);
    try {
      ...
      let errorMessage = 'İşlem eklenemedi.';
      ...
      <div className="atm-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="atm-transaction-form">
          <div className="atm-transaction-type-toggle">
            <span
              className={`atm-toggle-label ${!isExpense ? 'atm-active-income' : ''}`}
            >
              Income
            </span>
            ...
            <span
              className={`atm-toggle-label ${isExpense ? 'atm-active-expense' : ''}`}
            >
              Expense
            </span>
          </div>

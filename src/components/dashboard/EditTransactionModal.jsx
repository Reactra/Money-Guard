...
const EditTransactionModal = ({ onClose, transactionToEdit, categoryMap }) => {
  ...
  useEffect(() => {
    if (transactionToEdit && transactionToEdit.id) {
      ...
      if (isExpenseType && transactionToEdit.categoryId && categoryMap) {
        ...
        setCategory(categoryNameFromMap || 'Bilinmeyen Kategori');
      } else {
        setCategory('');
      }
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isExpense && (!category.trim() || category === 'Bilinmeyen Kategori')) {
      toast.error('Lütfen geçerli bir kategori belirtin.');
      return;
    }
    ...
    try {
      ...
      toast.error(err.data?.message || 'İşlem güncellenemedi.');
    }
  };
      ...
      <div className="etm-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="etm-transaction-form">
          <div className="etm-edit-toggle-group">
            <span
              className={`etm-edit-toggle-label ${!isExpense ? 'etm-active-income' : ''}`}
            ...
            <span
              className={`etm-edit-toggle-label ${isExpense ? 'etm-active-expense' : ''}`}
